...
if (isset($argv[1]) && is_file($argv[1]))
          ...
          elseif (!empty($options) && (!empty($options['c']) || !empty($options['check']))) {

                 if (is_file($options['c']) || is_file($options['check']))
                      (is_file($options['c']) ? read_filelist($options['c']) : read_filelist($options['check']));
                 else
                      (!empty($options['c']) ? read_array_filelist($options['c']) : read_array_filelist($options['check']));
          } else
