...
function _connect_server($_host,$_user,$_password,$_dbname) {
	global $_conn;
//	if(!$_conn = mysql_connect($_host, $_user, $_password)) {
	if(!$_conn = mysqli_connect ("$_host", "$_user", "$_password","$_dbname")) {
...
function _select_db($_dbname) {
	global $_conn;
	if(!mysqli_select_db($_conn,$_dbname)) {
		die('指定的数据库不存在！'.mysqli_errno());
	}
}
...
function _select_charset($_charset) {
	global $_conn;
	if(!mysqli_set_charset($_conn,$_charset)) {
		die('字符集设置错误！'.mysqli_errno());
	}
}
...
function _init_page($_sql, $_page_size) {
	...
	$GLOBALS['all_rows'] = mysqli_num_rows(mysqli_query($_conn,$_sql));
