...
if(isset($_GET['id'])) { //功能一:获取给定id的主题数据，并显示该主题的相关信息
	...
	if(_is_uniqid_consistent($_COOKIE['uniqid'], $_row['us_uniqid'])) {
		...
		if($_row) {
			...
			$_result = mysqli_query($_conn,"SELECT us_name,top_type,top_title,top_content,top_create_date FROM topic WHERE
	...
	if(_is_uniqid_consistent($_COOKIE['uniqid'], $_row['us_uniqid'])) {
		...
		if(mysqli_affected_rows($_conn) == 1) {
			$_data['time'] = _date();
			//更新用户最近一次的留言时间戳
			mysqli_query("UPDATE user SET last_reply_time='{$_data['time']}' WHERE us_name='{$_COOKIE['username']}'");
			//累积主题的留言数量和主题最近一次被访问的时间
			mysqli_query("UPDATE topic SET top_replycount=top_replycount+1,top_last_access_date='{$_data['time']}' WHERE reply_for=0 AND top_id='{$_data['reply_for']}'");
			mysqli_close();
	...
	<?php }?>
 	...
 	<?php
		...
		while (!!$_row = mysqli_fetch_array($_result)) {
			...
			global $_conn;
			$_resource = mysqli_query($_conn,"SELECT us_id,us_sex,us_face,us_email FROM user WHERE us_name='{$_data['reply_username']}' LIMIT 1");
			if (!!$_row = mysqli_fetch_array($_resource, MYSQLI_ASSOC)) {
				//提取留言用户的信息
				$_data['us_id'] = $_row['us_id'];
				$_data['sex'] = $_row['us_sex'];
				$_data['face'] = _html_chars($_row['us_face']);
				$_data['email'] = _html_chars($_row['us_email']);
				mysqli_free_result($_resource);
