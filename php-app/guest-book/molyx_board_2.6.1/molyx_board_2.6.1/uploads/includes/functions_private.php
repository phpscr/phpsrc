...
class functions_private
{
	...
	function sendpm()
	{
 		...
 		if (!$_INPUT['noredirect']) {
 			$this->credit->check_credit_limit('sendpm');
 		}
		...
		$post = $this->postlib->parser->censoredwords( $post );
 		...
 		$_INPUT['username'] = $forums->func->unclean_value($_INPUT['username']);
		$users = explode(';', $_INPUT['username']);
		$touser = array();
		foreach ($users as $val) {
			$val = trim($val);
			if ($val) {
				$touser[] = $forums->func->strtolower($forums->func->clean_value($val));
			}
		}
		...
		foreach ( $touser AS $username) {
			...
			if ( ! $user = $DB->query_first( "
						SELECT u.id, u.name, u.pmtotal, u.options, u.email, u.pmfolders, u.membergroupids, g.pmquota, p.id AS banid
						FROM ".TABLE_PREFIX."user u
						LEFT JOIN ".TABLE_PREFIX."usergroup g ON (u.usergroupid=g.usergroupid)
						LEFT JOIN ".TABLE_PREFIX."pmuserlist p ON (u.id=p.userid AND p.contactid={$bbuserinfo['id']} AND p.allowpm=0)
		...
		foreach ($touserlist AS $userid => $to_user) {
			$DB->query_unbuffered( "INSERT INTO ".TABLE_PREFIX."pm
								...
								VALUES
									(".$pmtextid.", ".TIMENOW.", '".addslashes($title)."', ".$bbuserinfo['id'].", ".$to_user['id'].", 0, ".intval($_INPUT['addtracking']).", ".intval($no_attachment).", ".$to_user['id'].")"
							);
			$pmid = $DB->insert_id();
			...
			if ($to_user['emailonpm']) {
				...
				$message = $this->email->fetch_email_pmnotify( array(
		...
		if (!$_INPUT['noredirect']) {
			$this->credit->update_credit('sendpm', $bbuserinfo['id']);
	...
	function newpm($errors='')
 	{
		...
		if ( $userid ) {
 		...
 		$title   = preg_replace( "/'/", "&#39;", $forums->func->stripslashes_uni($_INPUT['title']) );
		$content = $forums->func->htmlspecialchars_uni($forums->func->stripslashes_uni($_INPUT['post']));
 		if ( $getpmid ) {
			...
			$pm['message'] = $this->postlib->parser->unconvert($pm['message'],1,0,$bbuserinfo['usewysiwyg']);
 			if ($pm['title']) {
				if ($_INPUT['fwd'] == 1) {
 					...
 					$content   = $bbuserinfo['usewysiwyg'] ? $content : $forums->func->my_br2nl( $content );
 				} else {
		...
		$content = $forums->func->htmlspecialchars_uni($content);
		...
		$credit_list = $this->credit->show_credit('sendpm');
		...
		if ($bboptions['isajax']) {
			...
			$jsinclude['ajax'] = 1;
			$attachbutton = 'button'; $ajaxfunc = 'onclick="sendattach();"'; $previewfunc = 'onclick="previewpost(0);"';
		} else {
			$attachbutton = 'submit'; $ajaxfunc = "onclick='override=1;'"; $previewfunc = '';
		}
		$jsinclude['mxe'] = true;
	...
	function build_contact_list()
	{
		...
		$contact = "";
 		...
 		if ( $DB->num_rows() ) {
			$contact = "<select onchange='document.mxbform.username.focus(); document.mxbform.username.value = this.options[this.selectedIndex].value + document.mxbform.username.value;'>";
	...
	function rebuild_foldercount($userid, $folders, $curfolderid, $pmcount, $nosave='save', $extra="")
	{
		...
		if ( ! $folders ) {
			...
			$def_folders = array(
				'0' => array( 'pmcount' => 0, 'foldername' => $forums->lang['_inbox'] ),
