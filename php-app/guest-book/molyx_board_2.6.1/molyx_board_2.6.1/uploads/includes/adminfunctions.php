...
class admin_functions
{
	...
	function rm_dir( $file, $delfolder='1' )
	{
		$errors = 0;
	...
	function build_group_perms( $perms= array() )
	{
		...
		while ( $data=$DB->fetch_array() ) {
			...
			$this->print_cells_row( array(   "<div style='float:right'><input type='button' class='button' value='+' onclick='checkrow({$data['usergroupid']},1)' />&nbsp;<input type='button' class='button' value='-' onclick='checkrow({$data['usergroupid']},0)' /></div><strong>{$data['grouptitle']}</strong>",
	...
	function print_popup_header()
	{
		...
		echo "<script type=\"text/javascript\" src='../lang/{$bboptions['language']}/global.js'></script>\n";
		echo "<script type=\"text/javascript\" src='../clientscripts/global.js'></script>\n";
	...
	function menu()
	{
		...
		$forums->lang = $forums->func->load_lang($forums->lang, 'admin_nav' );
		if (is_file(ROOT_PATH."lang/{$bboptions['language']}/admin_nav_{$thispanel}.php")) {
			$forums->lang = $forums->func->load_lang($forums->lang, 'admin_nav_'.$thispanel );
		}
		echo "<script type=\"text/javascript\">\n";
		echo "<!--\n";
		echo "var cookie_id = \"{$bboptions['cookieprefix']}\";\n";
		...
		foreach($options AS $cat => $data) {
			...
			foreach($data AS $pid => $pdata) {
				...
				echo "<div class='menusub' id='m_".$thispanel.$cid."_{$pid}' onmouseout=\"change_cell_color('m_".$thispanel.$cid."_{$pid}', 'menusub');\" onmouseover=\"change_cell_color('m_".$thispanel.$cid."_{$pid}', 'menusubon');\">&nbsp;{$icon}&nbsp;<a href='".$link."' target='body' >".$forums->lang[$sub]."</a></div>\n";
			}
			echo "</div>\n";
			echo "</div>\n";
	...
	function nav()
	{
		...
		while ($file = @readdir($handle)) {
			if (preg_match("/^adminfunctions_nav_(.*)\./",$file,$regs)) {
				if (is_file(ROOT_PATH."lang/{$bboptions['language']}/admin_nav_{$regs[1]}.php")) {
					$forums->lang = $forums->func->load_lang($forums->lang, 'admin_nav_'.$regs[1] );
				}
				$panelfile = fopen(ROOT_PATH."includes/$file","r");
				fseek($panelfile,9);
				$name = $forums->func->htmlspecialchars_uni(str_replace("// ","",trim(fgets($panelfile,255))));
	...
	function print_form_header($hiddens="", $name='cpform', $js='', $script='', $method='post')
	{
		global $forums;
	...
	function print_input_row($name, $value="", $type='text', $js="", $size="30")
	{
	...
	function print_yes_no_row( $name, $default_val="", $extra='' )
	{
		...
		if ($extra) {
			$extra = "<input type='radio' name='{$name}' value='-1' />&nbsp;";
		}
		if ($default_val == 1) {
			$yes = "<input type='radio' name='{$name}' value='1' checked />&nbsp;".$forums->lang['yes']." &nbsp; ";
		} else if ($default_val == -1) {
			$extra  = "<input type='radio' name='{$name}' value='-1' checked />&nbsp;".$forums->lang['any'];
		} else  {
