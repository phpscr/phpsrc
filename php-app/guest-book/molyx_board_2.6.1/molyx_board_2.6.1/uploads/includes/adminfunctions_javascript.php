...
class adminfunctions_javascript
{
	function createjs($js=array(), $update=1, $oldname="")
	{
		...
		if ($js['type'] == 0) {
			...
			$query[] = "t.dateline < ".time();
			if($js['daylimit']) {
				$cuttime = time() - $js['daylimit']*86400;
			...
			if($DB->num_rows()) {
				while($t = $DB->fetch_array()) {
					...
					$htmlcode = str_replace("{c3}", $forums->cache['forum'][$t['forumid']]['name'], $htmlcode);
			...
			if ($forums->cache['credit']['list'][$js['selecttype']]) {
				$where = "ue.".$js['selecttype'];
			} else if ($js['selecttype'] == 0) {
				$where = "u.posts";
			} else if ($js['selecttype'] == 1) {
				$where = "u.cash";
			} else if ($js['selecttype'] == 2) {
				$where = "u.reputation";
		...
		if($js['export']) {
			...
			if ($output_code) {
				if (function_exists('mb_convert_encoding')) {
					$jscode =	mb_convert_encoding($jscode, $output_code, "UTF-8");
				} elseif (function_exists('iconv')) {
					$jscode =	iconv("UTF-8", $output_code, $jscode );
				} else {
					require_once( ROOT_PATH."includes/functions_encoding.php" );
					$this->convert = new Encoding();
					$jscode = preg_replace("#(\r\n|\r|\n)#", "<br />", $jscode);
					$jscode = $this->convert->EncodeString($jscode, "UTF-8", $output_code);
					$jscode = str_replace("<br />", "\r\n", $jscode);
				}
			}
		}
