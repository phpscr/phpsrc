...
class functions_email {
	...
	function functions_email($ROOT_PATH="")
	{
		global $forums, $DB, $bboptions;
		$forums->lang = $forums->func->load_lang($forums->lang, 'email' );
	...
	function send_mail()
	{
		...
		if ( ($this->from) AND ($this->subject) ) {
			...
			if (function_exists('mb_convert_encoding')) {
				$this->subject = mb_convert_encoding($this->subject, $this->char_set, 'UTF-8');
				$this->message = mb_convert_encoding($this->message, $this->char_set, 'UTF-8');
				$this->mail_headers = mb_convert_encoding($this->mail_headers, $this->char_set, 'UTF-8');
			} elseif (function_exists('iconv')) {
				$this->subject = iconv('UTF-8', $this->char_set, $this->subject );
				$this->message = iconv('UTF-8', $this->char_set, $this->message );
				$this->mail_headers = iconv('UTF-8', $this->char_set, $this->mail_headers );
			} else {
				if (!is_object($this->convert)) {
					include_once(ROOT_PATH.'includes/functions_encoding.php');
					$this->convert = new Encoding();
				}
				$this->subject = $this->convert->EncodeString($this->subject, 'UTF-8', $this->char_set);
				$this->message = preg_replace("#(\r\n|\r|\n)#", '<br />', $this->message);
				$this->message = $this->convert->EncodeString($this->message, 'UTF-8', $this->char_set);
				$this->message = str_replace('<br />', "\r\n", $this->message);
				$this->mail_headers = preg_replace("#(\r\n|\r|\n)#", "<br />", $this->mail_headers);
				$this->mail_headers = $this->convert->EncodeString($this->mail_headers, "UTF-8", $this->char_set);
				$this->mail_headers = str_replace("<br />", "\r\n", $this->mail_headers);
			}
			if ($this->emailtype != 'smtp') {
