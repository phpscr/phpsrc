...
class adminfunctions_importers {
	...
	function option2bin($string)
	{
		...
		if ($string=="yes" || $string=="is" || $string=="on" || $string=="true") {
			return 1;
		} elseif ($string=="no" || $string=="is not" || $string=="off" || $$string=="false") {
			return 0;
		} else {
			return $string;
		}
	...
	function importuser($user=array(),$md5=0,$salt=1)
	{
		...
		if ($salt == "1") {
			$user['salt'] = $forums->func->generate_user_salt(5);
			$user['password'] = md5($user['password']);
		}
		...
		if (count($user)) {
			...
			$forums->func->fetch_query_sql( $iuser, 'user');
		}
		$userid = $DB->insert_id();
		$userimported = sprintf( $forums->lang['userimported'], $forums->func->htmlspecialchars_uni($user['name']) );
	...
	function importpm($pm=array())
	{
		...
		if (count($pm)) {
			...
			$forums->func->fetch_query_sql( $ipmtext, 'pmtext');
			$ipm['messageid'] = $DB->insert_id();
			$pmid = $forums->func->fetch_query_sql( $ipm, 'pm');
		}
	...
	function importcategory($category=array())
	{
		...
		if (count($category)) {
			...
			$forums->func->fetch_query_sql( $icategory, 'forum');
		}
		$categoryid = $DB->insert_id();
		$categoryimported = sprintf( $forums->lang['categoryimported'], $forums->func->htmlspecialchars_uni($category['name']) );
	...
	function importforum($forum=array())
	{
		...
		$makeparent = new adminfunctions_forum();
		if(!is_array($fcachekeys)) {
			$fcachekeys = $this->describe("forum");
		}
		if (count($forum)) {
			$iforum = $this->dovalue($forum, $fcachekeys);
			$forums->func->fetch_query_sql( $iforum, 'forum');
		}
		$forumid = $DB->insert_id();
		$parentlist = $forumid . ',' . $makeparent->fetch_forum_parentlist($forum['parentid']);
		$DB->query_unbuffered("UPDATE ".TABLE_PREFIX."forum SET parentlist='$parentlist' WHERE id='$forumid'");
		$forumimported = sprintf( $forums->lang['forumimported'], $forums->func->htmlspecialchars_uni($forum['name']) );
	...
	function importmoderator($moderator=array())
	{
		...
		if (count($moderator)) {
			...
			$forums->func->fetch_query_sql( $imoderator, 'moderator');
		}
		$moderatorid = $DB->insert_id();
		$moderatorimported = sprintf( $forums->lang['moderatorimported'], $forums->func->htmlspecialchars_uni($moderator['username']) );
	...
	function importthread($thread=array())
	{
		...
		if (count($thread)) {
			...
			$forums->func->fetch_query_sql( $ithread, 'thread');
		}
	...
	function importpost($post=array())
	{
		...
		if (count($post)) {
			...
			$forums->func->fetch_query_sql( $ipost, 'post');
		}
		$postid = $DB->insert_id();
	...
	function importattachment($attachment=array())
	{
		...
		if (count($attachment)) {
			...
			$forums->func->fetch_query_sql( $iattachment, 'attachment');
		}
		$attachmentid = $DB->insert_id();
		$attachimported = sprintf( $forums->lang['attachimported'], $forums->func->htmlspecialchars_uni($attachment['filename']) );
	...
	function importpoll($poll=array())
	{
		...
		if (count($poll)) {
			...
			$forums->func->fetch_query_sql( $ipoll, 'poll');
		}
		$pollid = $DB->insert_id();
