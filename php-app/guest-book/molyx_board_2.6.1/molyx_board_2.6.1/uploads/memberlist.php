...
class memberlist {
    ...
    function show()
    {
		...
		$forums->lang = $forums->func->load_lang($forums->lang, 'member' );
    	...
    	foreach( $forums->cache['usergroup'] AS $id => $row ) {
    		...
    		$this->usergroup[ $row['usergroupid'] ] = array( 'title'  => $row['grouptitle'], 'icon'   => $row['groupicon'] );
    	}
    	...
    	foreach( $userinfo AS $in => $tbl ) {
    		$inbit = $forums->func->clean_value(trim(rawurldecode($_INPUT[ $in ])));
    		if ($in != 'name' && $in != 'gender') {
    			$url[] = $in.'='.$_INPUT[ $in ];
    		}
    		if ( $in == 'name' AND $inbit != "" ) {
				if ( $_INPUT['name_box'] == 'begins' ) {
					$query[] =  "LOWER(u.name) LIKE concat('".$forums->func->strtolower($inbit)."','%')";
					$url[] = 'name_box=begins';
				} else {
					$query[] = "LOWER(u.name) LIKE concat('%','".$forums->func->strtolower($inbit)."','%')";
				}
		...
		include $forums->func->load_template('memberlist');
		exit;
 	}
}
