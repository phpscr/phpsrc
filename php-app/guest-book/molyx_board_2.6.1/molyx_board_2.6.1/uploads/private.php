	...
	function pmlist()
 	{
		...
		$curfolderid = $this->folderid;
 		...
 		$pages = $forums->func->build_pagelinks(array(
 			'totalpages'  => $currenttotal['pmtotal'],
 			'perpage'    => 30,
 			'curpage'  => $pp,
	...
	function editfolders()
 	{
		...
		$curfolders = $bbuserinfo['pmfolders'];
 		...
 		for ($i = $count,$n = $count+3; $i < $n; $i++) {
			$newfolder .= "<p><input type='text' name='folder[".$i."]' value='' title='Folder id: ".$i."' /></p>\n";
 		}
 	...
 	function savefolders()
 	{
 		...
 		foreach ($_INPUT['folder'] AS $folderid => $foldername) {
			...
			$foldername = $forums->func->htmlspecialchars_uni(trim($foldername));
			if ($foldername != '') {
	...
	function buddylist()
 	...
 	function adduser()
 	{
		...
		$username = trim($_INPUT['username']);
 		...
 		$user = $DB->query_first( "SELECT name, id FROM ".TABLE_PREFIX."user WHERE LOWER(name)='".$forums->func->strtolower($username)."'" );
 		if (! $user['id']) {
	...
	function showpm()
 	{
		...
		$this->lib->postlib = new functions_post();
		...
		$user['grouptitle'] = $pm['grouptitle'];
		$pm['message'] = $this->lib->postlib->parser->convert_text( $pm['message'] );
		if ( $pm['attach'] ) {
 			...
 			if ($_INPUT['delete']) {
				...
				if ( intval($_INPUT['curfolderid'] != $this->folderid) ) {
					...
					if ( $DB->affected_rows() ) {
						...
						$returnpmfolders = unserialize(stripslashes($returnpmfolders));
