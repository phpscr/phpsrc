...
class faq {
    ...
    function show()
    {
		...
		$forums->lang = $forums->func->load_lang($forums->lang, 'faq' );
    	switch($_INPUT['do'])
    	{
    		case 'content':
				$show['content'] = TRUE;
				...
				$nav = array( "<a href='faq.php?{$forums->sessionurl}'>".$forums->lang['faq']."</a>", $this->result['title'] );
    			break;
    		case 'search':
				$show['search'] = TRUE;
		...
		include $forums->func->load_template('faq');
		exit;
 	}
 	...
 	{
 		global $forums, $DB, $_INPUT;
		$this->search = $forums->func->htmlspecialchars_uni(trim($_INPUT['q']));
 		...
 		$this->search = $forums->func->strtolower( str_replace( "*" , "%", $this->search ) );
