...
class sendmessage {
	...
	function show()
    {
		...
		$forums->lang = $forums->func->load_lang($forums->lang, 'sendmessage' );
	...
	{
		global $forums, $DB, $bbuserinfo, $_INPUT, $_USEROPTIONS, $bboptions;
		...
		if (!$user = $DB->query_first( "SELECT id, name, email, options FROM ".TABLE_PREFIX."user WHERE id=".$_INPUT['u']."" ))
		{
			$forums->func->standard_error("cannotfindmailer");
		}
		...
		if ($user['hideemail']) {
			$forums->func->standard_error("cannotmailuser", 0, $user['name']);
		}
		...
		if ( ! $_INPUT['send'] OR $errors ) {	
			...
			include $forums->func->load_template('mailmember');
			exit;
		} else {
	...
	function domailmember($user)
	{
		...
		$message = $this->email->fetch_email_mailmember( array(
												'message'     => preg_replace( "#<br.*>#siU", "\n", str_replace( "\r", "", $forums->func->stripslashes_uni($_POST['message']) ) ),
												'username' => $user['name'],
    ...
    function sendtofriend()
    {
		...
		include $forums->func->load_template('sendtofriend');
		exit;
	}
	...
	function dosend()
	{
		...
		$to_email = $forums->func->clean_email($_INPUT['to_email']);		
		if ( !$to_email ) {
