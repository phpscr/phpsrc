...
class filter {
	...
	function ban_add()
	{
		...
		$forums->func->fetch_query_sql( array( 'type' => $_INPUT['type'], 'content' => $_INPUT['content'] ), 'banfilter' );
		$this->func->banfilter_recache();
	...
	function badword_start()
	{
		...
		if ( $DB->num_rows() ) {
			...
			foreach($words AS $idx => $r) {
				$replace = $r['badafter'] ? stripslashes($r['badafter']) : '******';
				$type  = $r['type'] ? $forums->lang['exactmatch'] : $forums->lang['partmatch'];
				$forums->admin->print_cells_row( array( stripslashes($r['badbefore']),
														  $replace,
														  $type,
	...
	function badword_doedit()
	{
		...
		$forums->func->fetch_query_sql( array( 'badbefore' => $_INPUT['badbefore'], 'badafter' => $_INPUT['badafter'], 'type' => $_INPUT['type'] ), 'badword', "id='".$_INPUT['id']."'"  );
		$this->func->badword_recache();
	...
	function badword_edit()
	{
		...
		$forums->admin->print_cells_row( array( $forums->admin->print_input_row('badbefore', stripslashes($r['badbefore']) ),
												  			     $forums->admin->print_input_row('badafter' , stripslashes($r['badafter']) ),
	...
	function badword_add()
	{
		...
		$forums->func->fetch_query_sql( array( 'badbefore' => $_INPUT['badbefore'], 'badafter' => $_INPUT['badafter'], 'type' => $_INPUT['type'] ), 'badword' );
		$this->func->badword_recache();
