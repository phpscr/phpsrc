...
class moderate {
	...
	function do_edit()
	{
		...
		$forums->func->fetch_query_sql(array(
			...
			), 'moderator', 'moderatorid='.intval($_INPUT['u']) );
		$this->cache->moderator_recache();
	...
	function add_mod()
	{
		...
		if ($_INPUT['mtype'] == 'group') {
			...
			$moderator['usergroupname']  = $group['grouptitle'];
			$moderator['isgroup']    = 1;
			$forums->lang['addgroupmod'] = sprintf( $forums->lang['addgroupmod'], $group['grouptitle'] );
			...
			if($user['usergroupid'] != 4 && $user['usergroupid'] != 6 && $user['usergroupid'] != 7) {
				$DB->shutdown_query( "UPDATE ".TABLE_PREFIX."user SET usergroupid=7 WHERE id=".$user['id']."" );
			}
		...
		foreach ($forumids AS $cartman) {
			...
			$forums->func->fetch_query_sql( $moderator, 'moderator' );
		}
	...
	function mod_form( $type='add' )
	{
		...
		if ($type == 'add') {
			...
			if ($_INPUT['mtype'] == 'group') {
				...
				$forums->lang['addmodgroupdesc'] = sprintf( $forums->lang['addmodgroupdesc'], $group['grouptitle'], $thenames );
		...
		$forums->admin->print_form_header(array(
			...
			7 => array('gname', $group['grouptitle']),
		));
	...
	function add_two()
	{
		...
		$DB->query( "SELECT id, name FROM ".TABLE_PREFIX."user WHERE LOWER(name) LIKE concat('".$forums->func->strtolower($_INPUT['username'])."','%') OR name LIKE concat('".$_INPUT['username']."','%')");
		if (! $DB->num_rows() ) {
