...
class cronadmin {
	...
	function docron()
	{
		...
		$forums->func->fetch_query_sql( array( 'nextrun' => $newdate ), 'cron', "cronid=".$cron['cronid'] );
	...
	function dosavecron($type='edit')
	{
		...
		if ( $type == 'edit' ) {
			$forums->func->fetch_query_sql( $save, 'cron', 'cronid='.$_INPUT['id'] );
			$forums->main_msg = $forums->lang['cronedited'];
		} else {
			$forums->func->fetch_query_sql( $save, 'cron' );
	...
	function show_crons()
	{
		...
		while ( $row = $DB->fetch_array() ) {
			...
			$class = "";
			$title = "";
			$next_run = $forums->func->get_date( $row['nextrun'], 2 );
			if ( $row['enabled'] != 1 ) {
