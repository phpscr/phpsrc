...
class bbcode {
	...
	function show()
	{
		...
		$this->func = new adminfunctions_cache();
		switch($_INPUT['do']) {
			case 'add':
	...
	function test()
	{
		...
		$text = $forums->func->convert_andstr($forums->func->stripslashes_uni($_POST['bbtest']));
	...
	function save($type='add')
	{
		...
		$bbcode = array('title'							=> $forums->func->convert_andstr( $forums->func->stripslashes_uni( $_POST['title'] )),
							'description'				=> $forums->func->convert_andstr($forums->func->stripslashes_uni( $_POST['description'] )),
							'bbcodetag'				=> $_INPUT['bbcodetag'],
							'bbcodereplacement'	=> $forums->func->convert_andstr($forums->func->stripslashes_uni( $_POST['bbcodereplacement'] )),
							'bbcodeexample'			=> $forums->func->convert_andstr($forums->func->stripslashes_uni( $_POST['bbcodeexample'] )),
							'twoparams'				=> $forums->func->convert_andstr($forums->func->stripslashes_uni( $_POST['twoparams'] )),
							'imagebutton'				=> $forums->func->convert_andstr($forums->func->stripslashes_uni( $_POST['imagebutton'] )) );
		if ( $type == 'add' ) {
			$forums->func->fetch_query_sql( $bbcode, 'bbcode' );
			$forums->main_msg = $forums->lang['bbcodeadded'];
		} else {
			$forums->func->fetch_query_sql( $bbcode, 'bbcode', 'bbcodeid='.$_INPUT['id'] );
	...
	function bbcode_form($type='add')
	{
		...
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['bbcodereplacement']."</strong><div class='description'>&lt;tag&gt;{content}&lt;/tag&gt;<br />&lt;tag='{option}'&gt;{content}&lt;/tag&gt;</div>", $forums->admin->print_textarea_row( 'bbcodereplacement', $_INPUT['bbcodereplacement'] ? $_INPUT['bbcodereplacement'] : $forums->func->htmlspecialchars_uni($bbcode['bbcodereplacement'] )) ) );
	...
	function showcode()
	{
		...
		while ( $row = $DB->fetch_array() ) {
			...
			$forums->admin->print_cells_row( array( "<strong>".$row['title']."</strong>",
																	 ...
																	 "<div align='center'>".
																	  $forums->admin->print_button($forums->lang['edit'], "bbcode.php?{$forums->sessionurl}do=edit&amp;id={$row['bbcodeid']}").'&nbsp;'.
																	  $forums->admin->print_button($forums->lang['delete'], "bbcode.php?{$forums->sessionurl}do=delete&amp;id={$row['bbcodeid']}")
																	 ."</div>",
														   )      );
		}
