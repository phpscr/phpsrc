...
class sptopic {
	...
	function show()
	{
		...
		$forums->lang = $forums->func->load_lang($forums->lang, 'admin_sptopic' );
	...
	function doadd()
	{
		...
		if ($id) {
			$forums->func->fetch_query_sql($data, "specialtopic", "id=".$id);
		} else {
			$forums->func->fetch_query_sql($data, "specialtopic");
			$id = $DB->insert_id();
		}
