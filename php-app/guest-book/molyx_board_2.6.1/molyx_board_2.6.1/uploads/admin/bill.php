...
class adminlog {
	...
	function show()
	{
		...
		$forums->lang = $forums->func->load_lang($forums->lang, 'admin_bill' );
	...
	function step1()
	{
		...
		foreach($this->allforum AS $key => $value) {
			$forum_list[] = array( $value['id'], $forums->func->construct_depth_mark($value['depth'], '--').$value['name'] );
		}
	...
	function finish()
	{
		...
		$_POST['code'] = trim($forums->func->convert_andstr($forums->func->stripslashes_uni($_POST['code'])));
		...
		switch ($_INPUT['codetype'])
		{
			...
			case 3:
				...
				$code = array("flash_url" => $_INPUT['flash_url'], 
		...
		if (!$_INPUT['id']) {
			$forums->func->fetch_query_sql($array, 'ad');
		...
		if ($_INPUT['id']) {
			$forums->func->fetch_query_sql($array, 'ad', 'id='.$_INPUT['id']);
		} else {
			$forums->func->fetch_query_sql($array, 'ad');
		}
		...
		if ($DB->num_rows($ads)) {
			while ($ad = $DB->fetch_array($ads)) {
				...
				if ($ad['codetype'] == 0 OR $ad['codetype'] == 3) {
					...
					$ad_click = $ad['click'] ? $forums->func->fetch_number_format($ad['click']) : $forums->lang['noclick'];
				}
		...
		if (is_array($_INPUT['order'])) {
			...
			while ($ad = $DB->fetch_array($ads)) {
				...
				if ($ad['displayorder'] != $displayorder) {
					$DB->query_unbuffered( "UPDATE ".TABLE_PREFIX."ad SET displayorder = {$displayorder} WHERE id = ".$ad['id']."" );
				}
			}
		}
		$this->cache->ad_recache();
