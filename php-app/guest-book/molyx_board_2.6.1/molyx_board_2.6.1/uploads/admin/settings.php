...
class settings {
	...
	function setting_start()
	{
		...
		$forums->admin->print_cells_row( array( array("<div align='center' style='white-space:nowrap'>".
																	  $forums->admin->print_button($forums->lang['addsettinggroup'], "settings.php?{$forums->sessionurl}do=settinggroup_new")."</div>", 3, 'pformstrip' )
										 					  )      );
		$forums->admin->print_table_footer();
		$forums->admin->print_cp_footer();
	}
	...
	function group_recount($gid)
	{
		...
		if ( $gid ) {
			...
			$count = intval($conf['count']);
			$forums->func->fetch_query_sql( array( 'groupcount' => $count ), 'settinggroup', 'groupid='.$gid );
		}
	}
	...
	function settinggroup_form( $type='add' )
	{
		...
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['groupdesc'].":</strong>", $forums->admin->print_textarea_row( 'description', $_POST['description'] ? $forums->func->stripslashes_uni($_POST['description']) : $conf['description'] ) ) );
	...
	function savegroup($type='add')
	{
		...
		$array = array( 'title' => $_INPUT['title'], 'description' => $forums->func->convert_andstr($forums->func->stripslashes_uni( $_POST['description'] )) );
		if ( $type == 'add' ) {
			$forums->func->fetch_query_sql( $array, 'settinggroup' );
			$forums->main_msg = $forums->lang['settinggroupadded'];
		} else {
			$forums->func->fetch_query_sql( $array, 'settinggroup', 'groupid='.$_INPUT['id'] );
	...
	function setting_view()
	{
		...
		$entry = array();
		if ( $_INPUT['search'] ) {
			$keywords = $forums->func->strtolower($_INPUT['search']);
	...
	{
		global $forums, $DB, $_INPUT, $config;
		$form_element  = "";
		$dropdown      = array();
		$start         = "";
		$end           = "";
		...
		$tdrow2        = "tdrow2";
		$key   = $r['varname'];
		if ($r['varname'] == 'remoteattach') {
			$value = $config['remoteattach'] ? $config['remoteattach'] : ($r['value'] != "" ? $r['value'] : $r['defaultvalue']);
		} elseif ($r['varname'] == 'bburl') {
			$value = $config['forum_url'] ? $config['forum_url'] : ($r['value'] != "" ? $r['value'] : $r['defaultvalue']);
		} else {
	...
	function setting_update( $donothing="" )
	{
		...
		foreach($db_fields as $key => $data) {
			if ($key == 'remoteattach' OR $key == 'bburl') {
				$_POST['bburl'] = $_POST['bburl'] ? trim($_POST['bburl']) : $config['forum_url'];
				$_POST['remoteattach'] = $_POST['remoteattach'] ? trim($_POST['remoteattach']) : $config['remoteattach'];
				$fp = @fopen(ROOT_PATH.'includes/config.php', 'wb');
				fwrite($fp, '<?php
$config[\'servername\'] = "'.$config['servername'].'";
$config[\'dbname\'] = "'.$config['dbname'].'";
$config[\'dbusername\'] = "'.$config['dbusername'].'";
$config[\'dbpassword\'] = "'.$config['dbpassword'].'";
$config[\'tableprefix\'] = "'.$config['tableprefix'].'";
$config[\'usepconnect\'] = "'.$config['usepconnect'].'";
$config[\'superadmin\'] = "'.$config['superadmin'].'";
$config[\'technicalemail\'] = "'.$config['technicalemail'].'";
$config[\'dbtype\'] = "'.$config['dbtype'].'";
$config[\'dbcharset\'] = "'.$config['dbcharset'].'";
$config[\'forum_url\'] = "'.$_POST['bburl'].'";
$config[\'remoteattach\'] = "'.$_POST['remoteattach'].'";
?>');
				@fclose($fp);
			} else {
				...
				if ($key == 'threadviewsdelay' && $_POST[$key]) {
					$forums->func->fetch_query_sql(array('enabled' => 1), 'cron', "filename='threadviews.php'");
				} else if ($key == 'threadviewsdelay' && $_POST[$key]) {
					$forums->func->fetch_query_sql(array('enabled' => 1), 'cron', "filename='attachmentviews.php'");
				}
			}
		}
	...
	function setting_save($type='add')
	{
		...
		$array = array( 'title'			=> $_INPUT['title'],
							'description'	=> $forums->func->convert_andstr($forums->func->stripslashes_uni( $_POST['description'] )),
							'groupid'			=> intval($_INPUT['groupid']),
							'type'				=> $_INPUT['type'],
							'varname'		=> $_INPUT['varname'],
							'value'			=> $forums->func->convert_andstr($forums->func->stripslashes_uni( $_POST['value'] )),
							'defaultvalue'	=> $forums->func->convert_andstr($forums->func->stripslashes_uni( $_POST['defaultvalue'] )),
							'dropextra'		=> $forums->func->convert_andstr($forums->func->stripslashes_uni( $_POST['dropextra'] )),
							'displayorder'	=> intval( $_INPUT['displayorder'] ),
							'addcache'		=> intval( $_INPUT['addcache'] ),
						 );
		if ( $type == 'add' ) {
			$forums->func->fetch_query_sql( $array, 'setting' );
			$this->group_recount($_INPUT['groupid']);
			$forums->main_msg = $forums->lang['settingadded'];
		} else {
			$forums->func->fetch_query_sql( $array, 'setting', 'settingid='.$_INPUT['id'] );
