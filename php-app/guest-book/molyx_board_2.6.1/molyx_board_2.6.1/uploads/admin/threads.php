...
class threads {
	...
	function massoperate($type='move')
	{
		...
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['issticky']."</strong>", $forums->admin->print_yes_no_row("thread[issticky]", 0, 1) ) );
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['isquintessence']."</strong>", $forums->admin->print_yes_no_row("thread[isquintessence]", 0, 1) ) );
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['isclosed']."</strong>", $forums->admin->print_yes_no_row("thread[isclosed]", -1, 1) ) );
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['isvisible']."</strong>", $forums->admin->print_yes_no_row("thread[isvisible]", -1, 1) ) );
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['isredirect']."</strong>", $forums->admin->print_yes_no_row("thread[isredirect]", -1, 1) ) );
	...
	function dopruneuser()
	{
		...
		if (!$user = $DB->query_first( "SELECT id,name FROM ".TABLE_PREFIX."user WHERE LOWER(name)='".$forums->func->strtolower($username)."' OR name='".$username."'" )) {
		...
		if (!$_INPUT['finishprune']) {
			...
			$this->mod->thread_delete($threadids, 0);
	...
	{
		global $DB, $_INPUT, $forums, $bbuserinfo;
		$this->thread = unserialize($forums->func->stripslashes_uni($_POST['contains']));
		$this->fetch_thread_move_prune_sql();
		$fullquery = "
			SELECT t.tid, f.name
