...
class iptools {
	...
	function show_ips()
	{
		...
		if ( $_INPUT['userid'] ) {
			...
			if ( ! $user = $DB->query_first( "SELECT id, name, email, host FROM ".TABLE_PREFIX."user WHERE LOWER(name)='".$forums->func->strtolower($username)."' OR name='".$username."'" ) ) {
	...
	function show_index($username="")
	{
		...
		if ( $username == "" ) {
			$forums->admin->print_cells_row( array( "<strong>".$forums->lang['bynamesearchips']."</strong>" ,
													  $forums->admin->print_input_row( "name", $forums->func->stripslashes_uni($_INPUT['name']) )
											 )      );
		} else {
			$DB->query( "SELECT id, name FROM ".TABLE_PREFIX."user WHERE LOWER(name) LIKE concat('".$forums->func->strtolower($username)."','%') OR name LIKE concat('".$username."','%')" );
			if ( ! $DB->num_rows() ) {
		...
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['findips']."</strong>" ,
												   $forums->admin->print_input_row( "ip", $forums->func->stripslashes_uni($_INPUT['ip']) )
										  )      );
		$forums->admin->print_form_submit($forums->lang['viewipinfo']);
		$forums->admin->print_table_footer();
		$forums->admin->print_form_end();
