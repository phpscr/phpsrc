...
class usertools {
	...
	function sendmail()
	{
		...
		if ($_INPUT['glist']) {
			...
			$separate = str_replace("\n", "<br />", $forums->func->convert_andstr($forums->func->stripslashes_uni($_POST['separate'])));
		} else {
			if ( $_POST['email_title'] OR $_POST['email_contents'] ) {
				$email_title = trim($forums->func->convert_andstr($forums->func->stripslashes_uni($_POST['email_title'])));
				...
				$email_contents = $parse->convert( array(
					'text' => $forums->func->stripslashes_uni($forums->func->htmlspecialchars_uni($email_contents)),
					'allowsmilies' => 1,
					'allowcode' => 1,
					'allowhtml' => 1)
				);
		...
		foreach( array('name','email','host','website','qq','uc','popo','aim','icq','yahoo','msn','signature','posts','suspended', 'registered_first', 'registered_last','lastpost_first', 'lastpost_last', 'lastactivity_first', 'lastactivity_last','usergroupid','membergroupid') AS $bit ) {
			...
			if ( $_INPUT[ $bit ] ) {
				if ( in_array( $bit, $date_keys ) ) {
					...
					$name = "LOWER(u.name) LIKE concat('".$start_bit."','".$forums->func->strtolower($_INPUT[$bit])."','".$end_bit."') OR u.name LIKE concat('".$start_bit."','".$_INPUT[$bit]."','".$end_bit."')";
					$query[] = $name;
				} else {
		...
		$users = $DB->query("SELECT u.email, u.name, u.id, u.usergroupid, ua.useractivationid FROM ".TABLE_PREFIX."user u LEFT JOIN ".TABLE_PREFIX."useractivation ua ON (ua.userid = u.id AND ua.type = 2) ".$where." ORDER BY name$limit");
		while ( $user = $DB->fetch_array( $users ) ) {
			if ($_INPUT['glist']) {
