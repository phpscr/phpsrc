...
class attachment {
	...
	function show()
	{
		...
		$this->attachfolder = $DB->query_first( "SELECT s.imagefolder FROM ".TABLE_PREFIX."style s WHERE s.usedefault=1" );
		$this->attachfolder['imagefolder'] .= '/'.$bboptions['language'];
	...
	function dosearch()
	{
		...
		if ( $_INPUT['days'] ) {
			$day_break = time() - intval( $_INPUT['days'] * 86400 );
		...
		if ( $_INPUT['authorname'] ) {
			$user = $DB->query_first( "SELECT id FROM ".TABLE_PREFIX."user WHERE LOWER(name) LIKE '%".$forums->func->strtolower( $_INPUT['authorname'] )."%' OR name LIKE '%". $_INPUT['authorname']."%'" );
		...
		while ( $r = $DB->fetch_array() ) {
			...
			$forums->admin->print_cells_row( array( "<img src='../images/{$this->attachfolder['imagefolder']}/{$forums->cache['attachmenttype'][ $r['extension'] ]['attachimg']}' border='0' alt='' />" ,
																     ...
																     $forums->func->fetch_number_format($r['filesize'], true),
	...
	function search_form()
	{
		...
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['orderby']."</strong>",
																 $forums->admin->print_input_select_row( 'orderby', array(
																													0 => array( 'dateline', $forums->lang['uploadtime'] ),
																 										             1 => array( 'counter', $forums->lang['viewtimes'] ),
																 										             2 => array( 'filesize', $forums->lang['filesize'] ),
																 										             3 => array( 'filename', $forums->lang['filename'] ),
																 										           ), $_INPUT['orderby'] ).' '.
																 $forums->admin->print_input_select_row( 'sort', array( 0 => array( 'desc', $forums->lang['descending'] ),
	...
	function stats_form()
	{
		...
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['attachnums']."</strong>" , $forums->func->fetch_number_format($stats['count']) ) );
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['diskused']."</strong>", $forums->func->fetch_number_format($stats['sum'], true) ) );
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['attachaverage']."</strong>", $stats['count'] ? $forums->func->fetch_number_format(($stats['sum']/$stats['count']), true) : '0 '.$forums->lang['bytes'] ) );
		...
		while ( $r = $DB->fetch_array() ) {
			$r['title'] = strip_tags($r['title']);
			$r['stitle'] = $forums->func->fetch_trimmed_title($r['title'], 15);
			$size = $forums->func->fetch_number_format($r['filesize'], true);
			$forums->admin->print_cells_row( array( "<img src='../images/{$this->attachfolder['imagefolder']}/{$forums->cache['attachmenttype'][ $r['extension'] ]['attachimg']}' border='0' alt='' />" ,
																	 "<a href='../attachment.php?id={$r['attachmentid']}&amp;u={$r['userid']}&amp;extension={$r['extension']}&amp;attach={$r['location']}&amp;filename={$r['filename']}&amp;attachpath={$r['attachpath']}' target='_blank'>{$r['filename']}</a>",
																     $r['counter'],
																     "<a href='../profile.php?u={$r['userid']}' target='_blank'>{$r['username']}</a>",
	...
	function attachtypes_delete()
	{
		...
		$DB->query_unbuffered( "DELETE FROM ".TABLE_PREFIX."attachmenttype WHERE id=".$_INPUT['id']."" );
	...
	function attachtypes_save( $type='add' )
	{
		global $forums, $DB, $_INPUT;
		$_INPUT['id'] = intval($_INPUT['id']);
		if ( ! $_INPUT['extension'] OR ! $_INPUT['mimetype'] ) {
			$forums->main_msg = $forums->lang['mustinputmime'];
			$this->attachtypes_form( $type );
		}
		$save_array = array( 'extension' => str_replace( ".", "", $_INPUT['extension'] ),
							 'mimetype'  => $_INPUT['mimetype'],
							 'usepost'      => $_INPUT['usepost'],
							 'useavatar'     => $_INPUT['useavatar'],
							 'attachimg'       => $_INPUT['attachimg']
						   );
		if ( $type == 'add' ) {
			$attach = $DB->query_first( "SELECT * FROM ".TABLE_PREFIX."attachmenttype WHERE extension='".$save_array['extension']."'" );
			if ( $attach['id'] ) {
				$forums->lang['mimeexist'] = sprintf( $forums->lang['mimeexist'], $save_array['extension'] );
				$forums->main_msg = $forums->lang['mimeexist'];
				$this->attachtypes_form($type);
			}
			$forums->func->fetch_query_sql( $save_array, 'attachmenttype' );
			$forums->main_msg = $forums->lang['attachtypeadded'];
		} else {
			$forums->func->fetch_query_sql( $save_array, 'attachmenttype', 'id='.$_INPUT['id'] );
	...
	function attachtypes_form( $type='add' )
	{
		...
		if ( $type == 'add' ) {
			...
			if ( $_INPUT['istype'] ) {
				$attach = $DB->query_first( "SELECT * FROM ".TABLE_PREFIX."attachmenttype WHERE id=".$_INPUT['istype']."" );
			} else {
				$attach = array();
			}
			$types = "";
			$DB->query( "SELECT * FROM ".TABLE_PREFIX."attachmenttype ORDER BY extension" );
			while( $r = $DB->fetch_array() ) {
				$types .= "<option value='{$r['id']}'>".$forums->lang['baseon'].": {$r['extension']}</option>\n";
			}
			$extra = "<div style='float:right;width:auto;padding-right:3px;'>
					  <form method='post' action='attachment.php?{$forums->sessionurl}do=add'>
					  <select name='istype' class='button'>{$types}</select> &nbsp;<input type='submit' value='".$forums->lang['ok']."' class='button' />
					  </form>
					  </div>";
		} else {
			$code   = 'doedit';
			$title = $forums->lang['editattachtype'];
			$attach = $DB->query_first( "SELECT * FROM ".TABLE_PREFIX."attachmenttype WHERE id=".$_INPUT['id']."" );
			if ( ! $attach['id'] ) {
		...
		$forums->admin->columns[] = array( "&nbsp;" , "40%" );
		$forums->admin->columns[] = array( "&nbsp;" , "60%" );
		$createform = array( 1 => array( 'do'  , $code     ),
							 2 => array( 'id'    , $_INPUT['id'] )
						   );
		$forums->admin->print_table_start( $title, '', $extra, $createform);
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['extension']."</strong><div class='description'>".$forums->lang['extensiondesc']."</div>",
												 				 $forums->admin->print_input_row( 'extension', $_INPUT['extension'] ? $_INPUT['extension'] : $attach['extension'], '', '', 10 ),
														)      );
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['mimetype']."</strong><div class='description'>".$forums->lang['mimetypedesc']."</div>",
												 				 $forums->admin->print_input_row( 'mimetype', $_INPUT['mimetype'] ? $_INPUT['mimetype'] : $attach['mimetype'], 40 ),
														)      );
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['mimetypeusepost']."</strong>",
												 				 $forums->admin->print_yes_no_row( 'usepost', $_INPUT['usepost'] ? $_INPUT['usepost'] : $attach['usepost'] ),
														)      );
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['mimetypeuseavatar']."</strong>",
												 				 $forums->admin->print_yes_no_row( 'useavatar', $_INPUT['useavatar'] ? $_INPUT['useavatar'] : $attach['useavatar'] ),
														)      );
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['attachimages']."</strong><div class='description'>".$forums->lang['attachimagesdesc']."</div>",
	...
	function attachtypes_start()
	{
		...
		$forums->admin->columns[] = array( $forums->lang['mimetype'], "40%" );
		...
		$type = $DB->query( "SELECT * FROM ".TABLE_PREFIX."attachmenttype ORDER BY extension" );
		while( $r = $DB->fetch_array( $type ) ) {
			...
			$forums->admin->print_cells_row( array( "<img src='../images/{$this->attachfolder['imagefolder']}/{$r['attachimg']}' border='0' alt='' />",
