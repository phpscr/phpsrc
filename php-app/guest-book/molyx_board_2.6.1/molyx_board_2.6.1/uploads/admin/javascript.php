...
class javascript {
	...
	function show()
	{
		...
		$forums->lang = $forums->func->load_lang($forums->lang, 'admin_javascript' );
	...
	function doform($action='edit')
	{
		...
		foreach($allforum AS $forum) {
			$catelist[] = array( $forum[id], $forums->func->construct_depth_mark($forum['depth'], '--').$forum[name] );
		}
		...
		if (is_array($forums->cache['credit']['list'])) {
			foreach ($forums->cache['credit']['list'] AS $type => $name) {
				$s_type[] = array( $type, sprintf($forums->lang['type_credit'], $name));
				$c_ex_desc .= "<br />{{$type}} => {$name}";
			}
		}
		...
		$m_htmlcode = $_INPUT['m_htmlcode'] ? $forums->func->stripslashes_uni($_INPUT['m_htmlcode']) : $js['htmlcode'];
	...
	function updatejs()
	{
		...
		$htmlcode = $_INPUT['type'] ? trim($forums->func->convert_andstr($forums->func->stripslashes_uni($_POST['m_htmlcode']))) : trim($forums->func->convert_andstr($forums->func->stripslashes_uni($_POST['htmlcode'])));

		if ($refresh > 0) {
			$nextrun = time() + $refresh*60;
		} else {
			$nextrun = -1;
		}
		...
		if($id) {
			$forums->func->fetch_query_sql($js, "javascript", "id=".$id);
		} else {
			$forums->func->fetch_query_sql($js, "javascript");
		}
