...
class faq {
	...
	function doedit($type = 'add')
	{
		...
		$title  = $forums->func->convert_andstr($forums->func->stripslashes_uni($_POST['title'] ));
		if ($_INPUT['parentid']) {
			$text  = preg_replace( array("/\n/", "/\\\/"), array("<br />", "&#092;"), $forums->func->convert_andstr($forums->func->stripslashes_uni($_POST['text'] )) );
		}
		$desc  = preg_replace( "/\n/", "<br />", $forums->func->convert_andstr($forums->func->stripslashes_uni($_POST['description'] )) );
		if ($type == 'add') {
			$forums->func->fetch_query_sql( array( 'title' => $title, 'text' => $text, 'description' => $description, 'parentid' => $_INPUT['parentid'], 'displayorder' => $_INPUT['displayorder']  ), 'faq' );
			$forums->admin->save_log($forums->lang['addnewfaq']);
		} else {
			$forums->func->fetch_query_sql( array( 'title' => $title, 'text' => $text, 'description' => $description, 'parentid' => $_INPUT['parentid'], 'displayorder' => $_INPUT['displayorder']  ), 'faq', "id=".intval($_INPUT['id'])     );
	...
	function showform($type='new')
	{
		...
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['faqtitle']."</strong>", $forums->admin->print_input_row('title', stripslashes($r['title']) ) ) );
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['faqdesc']."</strong>", $forums->admin->print_textarea_row('description', stripslashes($r['description']) ) ) );
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['faqtext']."</strong>", $forums->admin->print_textarea_row('text', $forums->func->htmlspecialchars_uni($r['text']), "60", "10" ) ) );
	...
	function list_files()
	{
		...
		if ( $DB->num_rows($pfaqs) ) {
			while ( $pfaq = $DB->fetch_array($pfaqs) ) {
				$faqlist .= "<ul><li><a href='faq.php?{$forums->sessionurl}do=edit&amp;id=".$pfaq['id']."'><strong>".stripslashes($pfaq['title'])."</strong></a> <a href='faq.php?{$forums->sessionurl}do=edit&amp;id=".$pfaq['id']."'>[".$forums->lang['edit']."]</a> <a href='faq.php?{$forums->sessionurl}do=remove&amp;id=".$pfaq['id']."'>[".$forums->lang['delete']."]</a></li></ul>\n";
				...
				if ( $DB->num_rows($subfaqs) ) {
					...
					while ( $subfaq = $DB->fetch_array($subfaqs) ) {
						$faqlist .= "<li><a href='faq.php?{$forums->sessionurl}do=edit&amp;id=".$subfaq['id']."'><strong>".stripslashes($subfaq['title'])."</strong></a> <a href='faq.php?{$forums->sessionurl}do=edit&amp;id=".$subfaq['id']."'>[".$forums->lang['edit']."]</a> <a href='faq.php?{$forums->sessionurl}do=remove&amp;id=".$subfaq['id']."'>[".$forums->lang['delete']."]</a></li>";
					}
					$faqlist .= "</ul>\n";
				}
			}
