...
class pms {
		...
		if ( $DB->num_rows() ) {
			while ( $r = $DB->fetch_array() ) {
				...
				foreach ($sendgroup AS $ids) {
					...
					$grouptitle[] = $groups[ $ids ]['grouptitle'];
				}
	...
	function sendpm()
	{
		...
		$title = trim($_INPUT['title']);
 		$message = $lib->convert( array( 'text' => $forums->func->convert_andstr($forums->func->stripslashes_uni($_POST['message'])),
																	'allowsmilies' => 0,
																	'allowcode' => 1,
																	'allowhtml' => 1,
																	'usewysiwyg' => 1,
		...
		if (!$_INPUT['id']) {
			...
			$DB->query_unbuffered( "INSERT INTO ".TABLE_PREFIX."pm
									(messageid, dateline, title, usergroupid)
								VALUES
									(".$pmtextid.", ".TIMENOW.", '".$title."', '".implode(',',$_INPUT['usergroupid'])."')"
							);
			$forums->admin->save_log($forums->lang['sendnewpms']);
			$forums->func->standard_redirect("pms.php?".$forums->sessionurl);
		} else {
			$DB->query_unbuffered( "UPDATE ".TABLE_PREFIX."pm SET title = '".addslashes($title)."', usergroupid='".implode(',',$_INPUT['usergroupid'])."' WHERE pmid=".intval($_INPUT['id'])."");
			$DB->query_unbuffered( "UPDATE ".TABLE_PREFIX."pmtext SET message = '".addslashes($message)."' WHERE pmtextid=".intval($_INPUT['messageid'])."");
