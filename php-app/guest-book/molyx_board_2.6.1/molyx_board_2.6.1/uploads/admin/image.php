			...
			if ( preg_match( "/^smile_type_(\d+)$/", $key, $match ) ) {
				if ( isset( $_INPUT[$match[0]]) ) {
					...
					if ( $smiletext AND $match[1] ) {
						$forums->func->fetch_query_sql( array( 'displayorder' => intval($displayorder), 'smiletext' => $smiletext ), 'smile', 'id='.$match[1] );
					}
				}
			}
		}
			...
			if ( preg_match( "/^smile_type_(\d+)$/", $key, $match ) ) {
				if ( isset( $_INPUT[$match[0]]) ) {
					...
					if ( $add AND $smiletext AND $image ) {
						$forums->func->fetch_query_sql( array( 'smiletext' => $smiletext, 'image' => $image ), 'smile' );
					}
				}
			}
		}
			...
			if ( preg_match( "/^icon_type_(\d+)$/", $key, $match ) ) {
				if ( isset( $_INPUT[$match[0]]) ) {
					...
					if ( $icontext AND $match[1] ) {
						$forums->func->fetch_query_sql( array( 'displayorder' => intval($displayorder), 'icontext' => $icontext ), 'icon', 'id='.$match[1] );
					}
				}
			}
		}
		...
		foreach ($_INPUT AS $key => $value) {
			if ( preg_match( "/^icon_type_(\d+)$/", $key, $match ) ) {
				if ( isset( $_INPUT[$match[0]]) ) {
					...
					if ( $add AND $icontext AND $image ) {
						$forums->func->fetch_query_sql( array( 'icontext' => $icontext, 'image' => $image ), 'icon'  );
					}
				}
			}
		}
