...
class league {
	...
	function league_form($type='edit')
	{
		...
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['sitename']."</strong>", $forums->admin->print_input_row("sitename", $forums->func->htmlspecialchars_uni($league['sitename'])) ) );
	...
	function updateleague()
	{
		...
		$leagueid = intval($_INPUT['id']);
		$league = array('sitename' => $_INPUT['sitename'],
		...
		if ($leagueid) {
			$forums->func->fetch_query_sql($league, 'league', "leagueid = $leagueid");
		} else {
			$forums->func->fetch_query_sql($league, 'league');
		}
