...
class reputation {
	...
	function replist()
	{
		...
		$row = $DB->query_first( "SELECT COUNT(pid) as count FROM ".TABLE_PREFIX."post WHERE reppost!=''".$query."" );
		$row_count = $row['count'];
		...
		$reputations = $DB->query( "SELECT p.pid,p.pagetext,p.reppost,p.threadid,u.id,u.name,u.reputation,t.title FROM ".TABLE_PREFIX."post p LEFT JOIN ".TABLE_PREFIX."user u ON (p.userid=u.id) LEFT JOIN ".TABLE_PREFIX."thread t ON (p.threadid=t.tid) WHERE reppost!=''".$query." ORDER BY p.pid DESC LIMIT ".$pp.", 20" );
		if ( $DB->num_rows($reputations) ) {
			while ( $reputation = $DB->fetch_array($reputations) ) {
				...
				$forums->admin->print_cells_row( array( 
					"<a href='../redirect.php?t=".$reputation['threadid']."&amp;goto=findpost&amp;p=".$reputation['pid']."' target='_blank'>".$reputation['pid']."</a>", 
		...
		$form_array = array(
							  0 => array( 'pid', $forums->lang['postid'] ),
	...
	function resetrep()
	{
		...
		$DB->query_unbuffered("UPDATE ".TABLE_PREFIX."post SET reppost = '".$reputation."' WHERE pid = ".$_INPUT['id']."");
