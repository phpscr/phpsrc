...
class usergroup {
	...
	function show()
	{
		...
		$this->forumcache = new adminfunctions_cache();
	...
	function userpromotion()
	{
		...
		$getpromos = $DB->query(" SELECT up.*, u.grouptitle 
													...
													LEFT JOIN " . TABLE_PREFIX . "usergroup u ON (up.joinusergroupid = u.usergroupid)$gquery"
												);
		...
		if ( count($promotions) > 0 ) {
			...
			foreach($promotions AS $groupid => $promos) {
				$forums->admin->print_cells_single_row( $forums->lang['pusergroup']." - ".$forums->cache['usergroup'][$groupid]['grouptitle']."", "left", "pformstrip" );
				foreach($promos AS $promotion) {
					...
					$forums->admin->print_cells_row( array( "<strong>".$promotion['grouptitle']."</strong>" ,
	...
	function adduserpromotion($type='edit')
	{
		...
		while ( $r = $DB->fetch_array() ) {
			$usergroup[] = array( $r['usergroupid'] , $r['grouptitle'] );
		}
		if ($type == 'add') {
			...
			$promotion = $DB->query_first("SELECT up.*, u.grouptitle 
																FROM ".TABLE_PREFIX."userpromotion up,
																".TABLE_PREFIX."usergroup u
																WHERE up.userpromotionid = ".$_INPUT['id']." AND up.usergroupid = u.usergroupid"
														);
		...
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['reputation']."</strong>", $forums->admin->print_input_select_row('reputation_sign', $signlist, $promotion['reputation_sign']).$forums->admin->print_input_row( "reputation", $promotion['reputation'] ) ) );
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['promotionstrategy']."</strong><div class='description'>".$forums->lang['promotionstrategydesc']."</div>",$forums->admin->print_input_select_row( 'strategy', 
	...
	function updatepromotion()
	{
		...
		if (!empty($_INPUT['userpromotionid'])) {
			$forums->func->fetch_query_sql($promotion, 'userpromotion', 'userpromotionid='.intval($_INPUT['userpromotionid']));
		} else {
			$forums->func->fetch_query_sql($promotion, 'userpromotion');
		}
	...
	function previewforums()
	{
		...
		$forums->admin->print_table_start( $forums->lang['viewpermsgroup'].": " . $perms['grouptitle'] );
		$html = "";
		$perm_id = intval($_INPUT['id']);
		$allforum = $forums->func->cache_forums();
		foreach($allforum AS $key => $value) {
			$theforums[] = array( $value[id], $forums->func->construct_depth_mark($value['depth'], '--').$value[name] );
		}
	...
	function permission()
	{
		...
		while( $r = $DB->fetch_array() ) {
			$perms[ $r['usergroupid'] ] = $r['grouptitle'];
		}
	...
	function forumpermission()
	{
		...
		foreach( $allforum AS $id => $r ) {
			...
			$forums->admin->print_cells_row( array(
																	 "<div style='float:right'><input type='button' class='button' value='+' onclick='checkrow({$r['id']},1)' />&nbsp;<input type='button' class='button' value='-' onclick='checkrow({$r['id']},0)' /></div><strong>".$r['name']."</strong>",
	...
	function doforumpermission()
	{
		...
		while ( $row = $DB->fetch_array($forumperms) ) {
			...
			$forums->func->fetch_query_sql( array( 'permissions' => serialize(array(
	...
	function deleteform()
	{
		...
		$pagetitle = $forums->lang['deleteusergroup']." - ".$group['grouptitle'];		
		$detail = $forums->lang['deleteusergroupdesc'];
		$forums->admin->print_cp_header($pagetitle, $detail);
		$forums->admin->print_form_header( array( 1 => array( 'do', 'dodelete' ), 2 => array( 'id', $_INPUT['id'] ), 3 => array( 'grouptitle', $group['grouptitle'] ) ) );
		$forums->admin->columns[] = array( "&nbsp;"  , "40%" );
		$forums->admin->columns[] = array( "&nbsp;"  , "60%" );
		$forums->admin->print_table_start( $forums->lang['deleteusergroup']." - ".$group['grouptitle'] );
	...
	function dodeletegroup()
	{
		...
		$DB->query( "SELECT usergroupid FROM ".TABLE_PREFIX."usergroup WHERE usergroupid IN(".$_INPUT['id'].",".$_INPUT['to_id'].")" );
		if ( $DB->num_rows() != 2 ) {
			$forums->admin->print_cp_error($forums->lang['noids']);
		}
		$DB->query_unbuffered( "UPDATE ".TABLE_PREFIX."user SET usergroupid=".$_INPUT['to_id']." WHERE usergroupid=".$_INPUT['id']."" );
		$DB->query_unbuffered( "DELETE FROM ".TABLE_PREFIX."usergroup WHERE usergroupid=".$_INPUT['id']."" );
		$DB->query_unbuffered( "DELETE FROM ".TABLE_PREFIX."moderator WHERE isgroup=1 AND usergroupid=".$_INPUT['id']."" );
		$forums->admin->save_log($forums->lang['usergroupedited']." - '{$_INPUT['grouptitle']}'");
	...
	function savegroup($type='edit')
	{
		...
		$opentag = preg_replace( "/&#39;/", "'" , $forums->func->convert_andstr($forums->func->stripslashes_uni($_POST['opentag']) ) );
		$opentag = preg_replace( "/&lt;/" , "<" , $opentag );
		$closetag = preg_replace( "/&#39;/", "'" , $forums->func->convert_andstr($forums->func->stripslashes_uni($_POST['closetag'])) );
		...
		if ( isset($_INPUT['bankloanlimit']) && intval($_INPUT['bankloanlimit']) < 0 ) {
			$forums->main_msg = $forums->lang['bankloanlimiterror'];
			$this->groupform('edit');
		}
		$_INPUT['p_width']  = str_replace( ":", "", $_INPUT['p_width'] );
		$_INPUT['p_height'] = str_replace( ":", "", $_INPUT['p_height'] );
		if ($_INPUT['repgroupids'] == '' OR in_array(-1,$_INPUT['repgroupids'])) {
			$repgroupids = '';
		} else {
			$repgroupids = implode( ",", $_INPUT['repgroupids'] );
		}
		$usergroup = array(
							 'grouptitle'				=> trim($_INPUT['grouptitle']),
							 'groupicon'			=> trim($forums->func->convert_andstr($forums->func->stripslashes_uni($_POST['groupicon']))),
							 'onlineicon'			=> trim($forums->func->convert_andstr($forums->func->stripslashes_uni($_POST['onlineicon']))),
							 ...
							 'opentag'				=> $opentag,
							 'closetag'				=> $closetag,
							 'groupranks'			=> $forums->func->convert_andstr($forums->func->stripslashes_uni($_POST['groupranks'])),
							 ...
							 'bankloanlimit'			=> $_INPUT['bankloanlimit'],
							 'canmodrep'			=> $repgroupids,
							 'attachnum' => intval($_INPUT['attachnum']),
						  );
		if ($type == 'edit') {
			$forums->func->fetch_query_sql( $usergroup, 'usergroup', 'usergroupid='.$_INPUT['id'] );
			$DB->query_unbuffered( "UPDATE ".TABLE_PREFIX."moderator SET usergroupname='".trim($_INPUT['grouptitle'])."' WHERE usergroupid =".$_INPUT['id']."" );
			$forums->admin->save_log($forums->lang['usergroupedited']." - '{$_INPUT['grouptitle']}'");
			$this->rebuildgroup();
			$forums->admin->redirect("usergroup.php", $forums->lang['manageusergroup'], $forums->lang['usergroupedited'] );
		} else {
			$forums->func->fetch_query_sql( $usergroup, 'usergroup' );
			...
			$forums->admin->save_log($forums->lang['usergroupadded']." - '{$_INPUT['grouptitle']}'");
			$this->rebuildgroup();
	...
	function groupform($type='edit')
	{
		...
		$forums->admin->columns[] = array( "&nbsp;"  , "40%" );
		$forums->admin->columns[] = array( "&nbsp;"  , "60%" );
		$opentag = preg_replace( "/'/", "&#39;", $group['opentag'] );
		$opentag = preg_replace( "/</", "&lt;" , $opentag          );
		$closetag = preg_replace( "/'/", "&#39;", $group['closetag'] );
		$closetag = preg_replace( "/</", "&lt;" , $closetag          );
		$forums->admin->print_table_start( $forums->lang['basicsetting'], $forums->lang['basicsettingdesc'] );
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['usergrouptitle']."</strong>", $forums->admin->print_input_row("grouptitle", $group['grouptitle'] ) ) );
		...
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['usergrouprank']."</strong><div class='description'>".$forums->lang['usergrouprankdesc']."</div>", $forums->admin->print_input_row("groupranks", $group['groupranks'] ) ) );
		...
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['groupattachlimit']."</strong><div class='description'>".$forums->lang['groupattachlimitdesc']."</div>", $forums->admin->print_input_row("attachlimit", $group['attachlimit'] ). ' ('.$forums->lang['convert_value'].': '.($group['attachlimit'] == 0 ? $forums->lang['nolimit'] : $forums->func->fetch_number_format( $group['attachlimit'] * 1024, true )).')' ) );
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['perpostattach']."</strong><div class='description'>".$forums->lang['perpostattachdesc']."</div>", $forums->admin->print_input_row("perpostattach", $group['perpostattach'] ). ' ('.$forums->lang['convertvalue'].': '.($group['perpostattach'] == 0 ? $forums->lang['nolimit'] : $forums->func->fetch_number_format( $group['perpostattach'] * 1024, true ) ).')' ) );
		...
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['cancustomtitle']."</strong>", $forums->admin->print_yes_no_row("cancustomtitle", $group['cancustomtitle'] ) ) );
		...
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['canblog']."</strong>", $forums->admin->print_yes_no_row("canblog", $group['canblog'] ) ) );
		...
		$forums->admin->print_cells_row( array( "<strong>".$forums->lang['bankloanlimit']."</strong><div class='description'>".$forums->lang['bankloanlimitdesc']."</div>", $forums->admin->print_input_row("bankloanlimit", $group['bankloanlimit'] ) ) );
		...
		$forums->admin->print_table_footer();
		if ($group['usergroupid'] != 2) {
			$user_group[] = array( '-1' , $forums->lang['nomodrepperms'] );
			$DB->query( "SELECT usergroupid, grouptitle FROM ".TABLE_PREFIX."usergroup ORDER BY grouptitle" );
			while ( $r = $DB->fetch_array() ) {
				if ( $r['usergroupid'] == 4 ) {
					if ($bbuserinfo['usergroupid'] != 4) {
			...
			$arr = explode( ",", $group['canmodrep'] );
			$forums->admin->columns[] = array( "&nbsp;"  , "40%" );
			$forums->admin->columns[] = array( "&nbsp;"  , "60%" );
			$forums->admin->print_table_start( $forums->lang['grouprepperms'], $forums->lang['groupreppermsdesc'] );
			$forums->admin->print_cells_row( array( "<strong>".$forums->lang['canmodrep']."</strong><div class='description'>".$forums->lang['canmodrepdesc']."</div>", $forums->admin->print_multiple_select_row( "repgroupids[]", $user_group, $arr, 5 ) ) );
			$forums->admin->print_table_footer();
			$forums->admin->columns[] = array( "&nbsp;"  , "40%" );
			$forums->admin->columns[] = array( "&nbsp;"  , "60%" );
	...
	function mainform()
	{
		...
		$forums->admin->print_cp_header($pagetitle, $detail);
		$g_array = array();
		$forums->admin->columns[] = array( $forums->lang['usergrouptitle'], "30%" );
		$forums->admin->columns[] = array( $forums->lang['canadmincp'], "15%" );
		$forums->admin->columns[] = array( $forums->lang['supermoderator'], "15%" );
		...
		$result = $DB->query( "SELECT g.usergroupid, g.cancontrolpanel, g.supermod, g.grouptitle, g.opentag, g.closetag, COUNT(u.id) as count, g.displayorder
		...
		while ( $r = $DB->fetch_array($result) ) {
			...
			if ($r['usergroupid'] != 2 AND $r['usergroupid'] != 5) {
				$total_linkage = "<a href='../memberlist.php?max_results=30&amp;filter={$r['usergroupid']}&amp;order=asc&amp;sortby=name&amp;pp=0' target='_blank' title='".$forums->lang['userlist']."'>".$r['opentag'].$r['grouptitle'].$r['closetag']."</a>";
			} else {
				$total_linkage = $r['opentag'].$r['grouptitle'].$r['closetag'];
			}
			$forums->admin->print_cells_row( array( "<strong>$total_linkage</strong>" ,
												      ...
												      "<center><a href='usergroup.php?{$forums->sessionurl}do=edit&amp;id=".$r['usergroupid']."'>".$forums->lang['edit']."</a> $del</center>",
									     )      );
			$g_array[] = array( $r['usergroupid'], $r['grouptitle'] );
		}
		...
		$ids = array();
 		...
 		if (count($ids)) {
 			foreach($ids as $usergroupid => $new_position) {
 				$forums->func->fetch_query_sql(array('displayorder' => intval($new_position) ), 'usergroup', 'usergroupid='.$usergroupid);
 			}
 		}
 		$this->rebuildgroup();
