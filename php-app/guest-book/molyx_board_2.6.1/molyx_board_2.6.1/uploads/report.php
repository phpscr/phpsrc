...
class report {
    ...
    function show()
    {
		...
		$forums->lang = $forums->func->load_lang($forums->lang, 'report' );
		if (!$bbuserinfo['id']) {
	...
	function report_form()
	{
		...
		include $forums->func->load_template('report');
		exit;		
	}
	...
	function send()
	{
		...
		$this->check_permissions();
		$mods = array();
		$nmods = $DB->query( "SELECT u.id, u.name, u.email, m.moderatorid FROM ".TABLE_PREFIX."moderator m, ".TABLE_PREFIX."user u WHERE m.forumid=".$this->forumid." and m.userid=u.id" );
		if ( $DB->num_rows( $nmods ) ) {
			while( $r = $DB->fetch_array( $nmods ) ) {
				$mods[] = $r;
			}
		} else {
			$smods = $DB->query( "SELECT u.id, u.name, u.email FROM ".TABLE_PREFIX."user u, ".TABLE_PREFIX."usergroup g WHERE g.supermod=1 AND u.usergroupid=g.usergroupid" );
			if ( $DB->num_rows( $smods ) ) {
				while( $r = $DB->fetch_array( $smods ) ) {
					$mods[] = $r;
				}
			} else {
				$admin = $DB->query( "SELECT u.id, u.name, u.email FROM ".TABLE_PREFIX."user u, ".TABLE_PREFIX."usergroup g WHERE g.cancontrolpanel=1 AND u.usergroupid=g.usergroupid" );
