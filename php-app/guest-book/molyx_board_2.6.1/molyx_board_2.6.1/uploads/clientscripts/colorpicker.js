...
{
	var colorElement = my_getbyid("color_" + elm);
	var previewElement = my_getbyid("preview_" + elm);
...
{
	my_getbyid("sw" + x + "-" + y).style.backgroundColor = color;
}

function init_color_picker(setPickerType)
{
	colorPickerType = setPickerType;
	my_getbyid("colorPickerType").value = setPickerType;
...
function open_color_picker(clickedElementId, e)
{
	...
	pickerElement = my_getbyid("colorPicker");
	if (activeElementId == clickedElementId && pickerElement.style.display != "none")
	{
		...
		colorElement = my_getbyid("color_" + clickedElementId);
		previewElement = my_getbyid("preview_" + clickedElementId);
		my_getbyid("oldColor").style.background = previewElement.style.background;
		my_getbyid("newColor").style.background = previewElement.style.background;
		my_getbyid("txtColor").value = colorElement.value;
		if (!e) {
			e = window.event;
		}
		var scrollX = 0;
...
{
	activeElementId = 0;
	my_getbyid("colorPicker").style.display = "none";
}
...
function col_over(element)
{
	...
	my_getbyid("newColor").style.background = color;
	my_getbyid("txtColor").value = color;
}
...
function col_click(element)
{
	...
	my_getbyid("color_" + activeElementId).value = color;
