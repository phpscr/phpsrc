...
class rss {
    ...
    function show()
	{
		...
		$forums->lang = $forums->func->load_lang($forums->lang, 'rss');
		...
		if ( $_INPUT['fid'] == '' OR count($forumids)==0) {
		...
		while($thread = $DB->fetch_array($query)) {
			$thread['title'] = $forums->func->htmlspecialchars_uni(strip_tags($thread['title']));
			$thread['forumnanme'] = $forums->func->htmlspecialchars_uni(strip_tags($$thread['forumname']));
			$thread['lastposter'] = $forums->func->htmlspecialchars_uni($thread['lastposter']);
			...
			if ($thread['hidepost']) {
				...
				$pagetext = $forums->func->htmlspecialchars_uni($this->clean_message($thread['pagetext']));
