...
class newprivate
{
	...
	function showpm()
 	{
		...
		$pm['message'] = $this->postlib->parser->convert_text( $pm['message'] );
	...
	function sendpm()
	{
		...
		$_POST['post'] = $this->lib->postlib->parser->censoredwords( $_POST['post'] );
 		$message = $this->lib->postlib->parser->convert( array( 'text' => $forums->func->htmlspecialchars_uni($forums->func->stripslashes_uni(convert($_POST['post']))),
		...
		foreach ( $touser AS $username) {
			...
			if ( ! $user = $DB->query_first( "
						SELECT u.id, u.name, u.pmtotal, u.options, u.email, u.pmfolders, g.pmquota, p.id AS banid
						...
						WHERE LOWER(u.name)='".$forums->func->strtolower($username)."'" ) ) {
		...
		foreach ($touserlist AS $userid => $to_user) {
 		...
 		if ( $savecopy ) {
			...
			$forums->func->fetch_query_sql( array(	 'messageid'     => $pmtextid,
																	 'dateline'       => TIMENOW,
																	 'title'      => $title,
