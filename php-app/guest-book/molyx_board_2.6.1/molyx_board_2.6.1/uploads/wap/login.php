...
class login {
    ...
    function dologin()
    {
		...
		if ($sessionid) {
			$DB->query_unbuffered( "UPDATE ".TABLE_PREFIX."session SET username='".$user['name']."', userid=".$user['id'].", lastactivity=".TIMENOW.", usergroupid=".$user['usergroupid'].", invisible=".$invisible."  WHERE sessionhash='".$sessionid."'" );
		} else {
			$sessionid = substr(md5( uniqid(microtime())), 0, 16 );
			$DB->query_unbuffered("INSERT INTO ".TABLE_PREFIX."session  (sessionhash, username, userid, lastactivity, usergroupid, host, useragent, invisible) VALUES ('".$sessionid."', '".$user['name']."', '".$user['id']."', ".TIMENOW.", '".$user['usergroupid']."', '".$DB->escape_string(SESSION_HOST)."', '".$DB->escape_string(USER_AGENT)."', '".$invisible."')");
		}
		$bbuserinfo = $user;
	...
	function dologout()
	{
		...
		$DB->query_unbuffered("UPDATE ".TABLE_PREFIX."session SET username='', userid='0', invisible='0' WHERE sessionhash='". $forums->sessionid ."'");
