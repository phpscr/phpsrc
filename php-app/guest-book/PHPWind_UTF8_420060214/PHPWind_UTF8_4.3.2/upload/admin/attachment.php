...
if (empty($action)){
	...
	if ($postdate1){
		$uploadtime=strtotime($postdate1);
		is_numeric($uploadtime) && $sql.=" AND uploadtime > '$uploadtime'";
	}
	if ($postdate2){
		$uploadtime=strtotime($postdate2);
		is_numeric($uploadtime) && $sql.=" AND uploadtime < '$uploadtime'";
	}
				...
				while(false !==($file2=readdir($dir2))){
					if (is_file("$attachdir/$file1/$file2") && $file2!='' && $file2!='.' && $file2!='..' && !eregi("\.html$",$file2)){
						...
						if ($num > $start){
							...
							if ($num-$start >= $pernum){
								$start = $num-$delnum;
								adminmsg('delattach_step',"$basename&action=$action&start=$start&pernum=$pernum&deltotal=$deltotal",0);
							}
						}
					}
				}
...
function attachcheck($file){
	...
	if ($postdate1){
		$visittime=strtotime($postdate1);
		if (is_numeric($visittime) && fileatime("$attachdir/$file") < $visittime){
			return;
		}
	}
	if ($postdate2){
		$visittime=strtotime($postdate2);
		if (is_numeric($visittime) && fileatime("$attachdir/$file") > $visittime){
			return;
		}
	}
