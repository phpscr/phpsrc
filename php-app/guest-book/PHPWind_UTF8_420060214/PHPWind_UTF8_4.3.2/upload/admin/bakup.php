...
if($a_type=='bakout'){
	if(empty($action)){
		...
		if($stop==1){
			...
			adminmsg('bakup_step',"$basename&action=$action&start=$startfrom&tableid=$tableid&sizelimit=$sizelimit&step=$step&pre=$pre&tablesel=$tablesel&rows=$rows",2);
		} else{
			trim($writedata) && writeover(D_P.'data/'.$filename,$bak.$writedata,'ab');
			if($step>1){
				for($i=1;$i<=$f_num;$i++){
	...
	if(empty($action)){
		...
		*/
		...
		if($count > 1 && $step <= $count){
			adminmsg('bakup_in',"$basename&action=bakin&step=$step&count=$count&pre=$pre",2);
		}
		updatecache();
