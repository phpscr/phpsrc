...
if(!$step){
	...
	if (is_numeric($seekfid)){
		$rt = $db->get_one("SELECT forumadmin FROM pw_forums WHERE fid='$seekfid'");
		if ($windid == $manager){
			$admincheck = 1;
		} elseif ($rt['forumadmin'] && strpos($rt['forumadmin'],','.$windid.',') !== false){
			$admincheck = 1;
	...
	$keyword  && strlen($keyword) <= 2  && Showmsg('search_word_limit');
	...
	if (isset($_GET['sid']) && $_GET['sid']){
		...
		if (empty($schedid)){
			...
			if ($schedid && !$authorid){
				...
				//$sid = $db->insert_id();
				//$newatc && ObHeader("search.php?step=$step&sid=$sid&sch_time=all&orderway=lastpost&asc=desc&newatc=1&");
			}
		}
	}
	if ($schedid){
		...
		$pages = numofpage($total,$page,$numofpage,"search.php?step=$step&sid=$sid&keyword=$rawkeyword&pwuser=".rawurlencode($pwuser)."&authorid=$authorid&orderway=$orderway&s_type=$s_type&f_fid=$f_fid&c_fid=$c_fid&sch_time=$sch_time&sch_area=$sch_area&digest=$digest&");
