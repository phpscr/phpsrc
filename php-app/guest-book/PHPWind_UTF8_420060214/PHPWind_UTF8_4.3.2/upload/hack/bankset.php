...
$basename="$admin_file?adminjob=hack&hackset=bank";
...
if (!$action) {
	...
	if(isset($bk_A[$jifen1.'_'.$jifen2]) || isset($bk_A[$jifen2.'_'.$jifen1])){
	...
	if($rate1 > $rate2){
		adminmsg('bankset_rate_bug');
	}
	$bk_A[$jifen1.'_'.$jifen2]=array($c_types[$jifen1],$c_types[$jifen2],$rate1,$rate2,$method,$ifuse);
	updatecache_bk_A($bk_A);
	adminmsg('operate_success');
} elseif($action=="del"){
	unset($bk_A[$delid]);
	...
	$bk_A[$changeid][5]=$bk_A[$changeid][5]==1 ? 0 : 1 ;
	updatecache_bk_A($bk_A);
	adminmsg('operate_success');
} elseif($action=="method"){
	$bk_A[$mid][4]=$bk_A[$mid][4]==1 ? 0 : 1 ;
	updatecache_bk_A($bk_A);
	adminmsg('operate_success');
}
