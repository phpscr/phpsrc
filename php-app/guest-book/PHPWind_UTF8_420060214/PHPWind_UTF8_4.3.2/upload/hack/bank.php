...
if(empty($action) || $_POST['action']=='credit'){
	...
	foreach($creditdb as $key => $value){
		$showdb[$key]=array($value[0],$value[1]);
	}
}
if(empty($action)){
	if(!$bankdb){
	...
	if(!$bk_per || $timestamp - @filemtime(D_P."data/bbscache/bank_sort.php") > $bk_per*3600){
		$_DESPOSTDB=array();
		$query=$db->query("SELECT i.uid,username,deposit,startdate FROM pw_memberinfo i LEFT JOIN pw_members m ON m.uid=i.uid ORDER BY deposit DESC LIMIT $bk_num");
	...
	require_once(PrintEot('bank'));footer();
}
...
if($_POST['action']=='save'){
	...
	if(!$bk_A[$type] || !$bk_A[$type][5] || ($method==2 && !$bk_A[$type][4])){
		Showmsg('bk_credit_type_error');
	}
	if($method==1){
		list($sell,$buy)=explode('_',$type);
		$credit1=$change;
		$credit2=$change*$bk_A[$type][2];
 	} else{
		list($buy,$sell)=explode('_',$type);
		$credit1=$change*$bk_A[$type][3];
		$credit2=$change;
	}
