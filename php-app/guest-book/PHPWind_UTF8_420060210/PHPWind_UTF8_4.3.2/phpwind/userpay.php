...
if(!$action){
	...
	if((!$ol_payto || !$ol_md5code) && (!$ol_paypal || !$ol_paypalcode)){
	...
	$order_no = str_pad($winduid,10, "0",STR_PAD_LEFT).get_date($timestamp,'YmdHis').num_rand(5);
	$order_no = $method==1 ? '0'.$order_no : '1'.$order_no;
