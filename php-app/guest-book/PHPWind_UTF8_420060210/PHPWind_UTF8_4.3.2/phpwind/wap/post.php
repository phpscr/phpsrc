...
if($action=='new'){
	if(!$subject || !$content){
		...
		postcheck($fid,'new');
		$subject = U2GB(wap_cv($subject));
		$content = U2GB(wap_cv($content));
		...
		$lastpost=$subject."\t".$windid."\t".$timestamp."\t"."read.php?tid=$tid&page=e#a";
	...
	if(!$content){
		...
		$db->update("UPDATE pw_threads SET lastpost='$timestamp',lastposter ='$windid',replies=replies+1,hits=hits+1 WHERE tid='$tid'");

		$lastpost=$subject."\t".$windid."\t".$postdate."\t"."read.php?tid=$tid&page=e#a";
		$topicadd=",tpost=tpost+1,article=article+1,topic=topic+1";
