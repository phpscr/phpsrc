...
function AJAX_creat(){
	...
	Ajaxobj.value      = unhtmlspecialchars(linkobj.innerHTML);
	Ajaxobj.onblur     = AJAX_store;
	return Ajaxobj;
}
function AJAX_save(ajaxtext){
...
function AJAX_store(){
	if (editor_state == true){
		if (editobj.value != origsubject){
			AJAX_save(editobj.value);
			editobj.value     = htmlspecialchars(editobj.value)
			linkobj.innerHTML = editobj.value.replace(/\n/gi,'<br>');
		}else{
