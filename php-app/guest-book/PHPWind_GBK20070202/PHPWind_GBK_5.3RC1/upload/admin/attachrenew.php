...
if(empty($_POST['action'])){
	...
	foreach($ptable_a as $val){
		$query=$db->query("SELECT fid,tid,authorid,aid FROM $val WHERE aid<>''");
		while($aids=$db->fetch_array($query)){
			$attachs= unserialize(stripslashes($aids['aid']));
			if(is_array($attachs)){
				$update=0;
				...
				if($update){
					$attachs=$attachs ?  addslashes(serialize($attachs)):'';
					$db->update("UPDATE $val SET aid='$attachs' WHERE tid='$aids[tid]'");
				}
			} else{
				$count++;
				$db->update("UPDATE $val SET aid='' WHERE tid='$aids[tid]'");
			}
		}
	}
