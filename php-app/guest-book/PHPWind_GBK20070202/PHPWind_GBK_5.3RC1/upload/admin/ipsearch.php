...
if (empty($action)){
	...
	if($uids){
		...
		foreach($ptable_a as $val){
			$query=$db->query("SELECT userip,postdate AS lasttime,author AS username,authorid AS uid FROM $val WHERE userip!='' AND authorid IN($uids) GROUP BY userip");
			while($rt=$db->fetch_array($query)){
				$rt['lasttime']=get_date($rt['lasttime']);
				$ipdb[]=$rt;
			}
	...
	foreach($ptable_a as $val){
		$query=$db->query("SELECT authorid AS uid,author AS username,postdate AS lasttime,userip FROM $val tm WHERE $sql GROUP BY authorid");
		while($rt=$db->fetch_array($query)){
			$rt['lasttime']=get_date($rt['lasttime']);
			$userdb[]=$rt;
		}
